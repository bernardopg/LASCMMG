  ...
  async (req, res) => {
    try {
      ...
      if (honeypotLogs.length > MAX_LOGS_TO_PROCESS) {
        logger.warn(
          'SecurityRoute',
          `Honeypot log file is large (${honeypotLogs.length} entries). Processing last ${MAX_LOGS_TO_PROCESS} for overview stats.`
        );
...
router.post(
  ...
  authMiddleware,
  roleMiddleware('admin'),
  validateRequest(honeypotConfigSchema),
  async (req, res) => {
    ...
    try {
      ...
      if (success) {
        logger.info(
          ...
          'Configuração do honeypot atualizada.'
        );
        res.json({
          success: true,
        ...
        logger.error(
          { component: 'SecurityRoute', body: req.body, requestId: req.id },
...
router.post(
  ...
  authMiddleware,
  roleMiddleware('admin'),
  validateRequest(manualBlockIpSchema),
  async (req, res) => {
    ...
    try {
      ...
      if (success) {
        ...
        logger.warn(
          { component: 'SecurityRoute', ipAddress, requestId: req.id },
...
router.delete(
  ...
  authMiddleware,
  roleMiddleware('admin'), // Add roleMiddleware here
