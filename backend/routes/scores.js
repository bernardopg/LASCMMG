...
router.post('/', authMiddleware, validateRequest(newScoreSchema), async (req, res) => {
  ...
  const { tournamentId, matchId, player1Score, player2Score, winnerId, stateMatchKey } = req.body; // req.body is validated
  ...
  try {
    ...
    try {
      ...
      if (state && typeof state === 'object' && state.matches && state.matches[stateMatchKey]) {
        ...
        state.matches[stateMatchKey].score = [savedScore.player1_score, savedScore.player2_score];
        ...
        if (savedScore.winner_id !== null && savedScore.winner_id !== undefined) {
          ...
          // If no explicit winner_id, determine from scores
      ...
      res.status(201).json({
        success: true,
        message: 'Placar salvo, mas ocorreu um erro ao atualizar o estado do chaveamento.',
        score: savedScore,
    ...
    res.status(500).json({ success: false, message: 'Erro interno ao salvar placar.' });
  }
});
