...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ success: false, message: 'Erro ao atualizar jogador.' });
    }
  }
);
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ success: false, message: 'Erro ao excluir jogador.' });
    }
  }
);

// == SCORES MANAGEMENT ==
router.get('/scores', validateRequest(paginationQuerySchema), async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Erro ao buscar pontuações.' });
  }
});

router.post('/scores', validateRequest({ body: scoreUpdateSchema.body }), async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: 'Erro ao criar pontuação.' });
  }
});
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!updatedScore) {
        return res.status(404).json({ success: false, message: 'Pontuação não encontrada.' });
      }
      logger.info(
      ...
      res.status(500).json({ success: false, message: 'Erro ao atualizar pontuação.' });
    }
  }
);
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ success: false, message: 'Erro ao excluir pontuação.' });
    }
  }
);
...
router.get('/trash', validateRequest(adminGetTrashQuerySchema), async (req, res) => {
  try {
    ...
    if (!itemType || itemType === 'players') {
      ...
      trashItems = trashItems.concat(players.map((player) => ({ ...player, type: 'player' })));
      total += playersTotal;
    }
    ...
    if (!itemType || itemType === 'scores') {
      ...
      trashItems = trashItems.concat(scores.map((score) => ({ ...score, type: 'score' })));
      total += scoresTotal;
    }
    ...
    res.status(500).json({ success: false, message: 'Erro ao buscar itens da lixeira.' });
  }
});
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ success: false, message: 'Erro ao restaurar item.' });
    }
  }
);
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ success: false, message: 'Erro ao excluir item permanentemente.' });
    }
  }
);

// == IMPORT/UPLOAD MANAGEMENT ==
router.post('/import/players', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
    ...
    res.status(500).json({ success: false, message: 'Erro ao importar jogadores.' });
  }
});

router.post('/import/tournaments', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
    ...
    res.status(500).json({ success: false, message: 'Erro ao importar torneios.' });
  }
});

// == ADMIN USER MANAGEMENT ==
router.post('/users', validateRequest({ body: adminUserCreateSchema.body }), async (req, res) => {
...
router.get('/stats', async (req, res) => {
  try {
    const [tournamentStats, playerStats, scoreStats] = await Promise.all([
