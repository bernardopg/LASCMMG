...
const connectToRedis = async () => {
  ...
  попыткиПодключения++;
  logger.info('RedisClient', `Попытка подключения к Redis (${попыткиПодключения}/${МАКС_ПОПЫТКИ})... URL: ${REDIS_URL || 'redis://localhost:6379'}`);
  ...
  const client = redis.createClient({
    ...
    socket: {
      ...
      reconnectStrategy: (retries) => {
        if (retries >= 20) { // Stop retrying after 20 attempts for this strategy
          logger.error('RedisClient', 'Достигнуто максимальное количество попыток переподключения к Redis. Отключение.');
  ...
  client.on('error', (err) => {
    logger.error('RedisClient', 'Ошибка клиента Redis:', { error: err.message, stack: err.stack });
  ...
  try {
    ...
    logger.error('RedisClient', `Не удалось подключиться к Redis при запуске: ${err.message}`, { stack: err.stack });
    if (попыткиПодключения < МАКС_ПОПЫТКИ) {
      logger.info('RedisClient', `Повторная попытка подключения через ${ЗАДЕРЖКА_ПЕРЕПОДКЛЮЧЕНИЯ / 1000} секунд.`);
      ...
      logger.error('RedisClient', 'Достигнуто максимальное количество начальных попыток подключения к Redis. Сервер может работать с ограниченной функциональностью.');
...
const getClient = async () => {
  if (!redisClient || !redisClient.isOpen) {
    logger.warn('RedisClient', 'Клиент Redis не подключен или соединение закрыто. Попытка переподключения...');
