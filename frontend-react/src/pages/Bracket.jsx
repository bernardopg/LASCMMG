...
const Bracket = () => {
  ...
  useEffect(() => {
    const fetchBracketData = async () => {
      try {
        ...
        showError("Falha ao carregar dados do chaveamento", "Verifique sua conexão e tente novamente.");
        setLoading(false);
      }
    };
  ...
  const renderMatch = (match) => {
    ...
    const player1Class = hasWinner && match.winner === match.player1.id
      ? "bg-green-100 border-green-300"
      : "";

    const player2Class = hasWinner && match.winner === match.player2.id
      ? "bg-green-100 border-green-300"
      : "";
      ...
      >
        <div className={`player player1 p-3 border-b ${player1Class}`}>
          <div className="flex justify-between items-center">
            <span className="player-name font-medium">{match.player1?.name || "TBD"}</span>
            <span className="player-score font-bold">{isFinished ? match.score1 : "-"}</span>
          </div>
        </div>
        <div className={`player player2 p-3 ${player2Class}`}>
          <div className="flex justify-between items-center">
            <span className="player-name font-medium">{match.player2?.name || "TBD"}</span>
            <span className="player-score font-bold">{isFinished ? match.score2 : "-"}</span>
          </div>
        </div>
      </div>
    );
        ...
        <div className="bracket-header mb-6">
          ...
          <div className="bracket-controls flex flex-wrap justify-between items-center gap-2 print:hidden">
            <div className="bracket-info text-gray-600">
              {bracket && (
                <p>Visualizando {bracket.rounds.length} fases, {bracket.rounds.reduce((acc, round) => acc + round.matches.length, 0)} partidas</p>
              )}
            </div>
            ...
            <div className="bracket-actions flex flex-wrap items-center gap-2">
              ...
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M19,8A1,1 0 0,0 18,7H6V3H18V7A1,1 0 0,0 19,8M16,11H8V10H16V11M19,11A1,1 0 0,1 18,12H17V21H7V12H6A1,1 0 0,1 5,11A1,1 0 0,1 6,10H18A1,1 0 0,1 19,11M15,14H9V15H15V14M15,17H9V18H15V17Z"></path>
                </svg>
                <span>Imprimir</span>
              </button>
              ...
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M15 5H5V15H9V19H19V9H15V5M15 15H19V19H15V15Z"></path>
                </svg>
                <span>{printMode ? 'Modo Visualização' : 'Modo Impressão'}</span>
              </button>
            </div>
          </div>
        </div>
