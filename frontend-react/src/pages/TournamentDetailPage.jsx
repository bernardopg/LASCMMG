...
import { FaCalendarAlt, FaUsers, FaInfoCircle, FaMoneyBillWave, FaListOl, FaSitemap, FaEdit } from 'react-icons/fa';
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-[calc(100vh-10rem)]">
        ...
        <p className="ml-4 text-lg text-gray-700 dark:text-gray-200">Carregando detalhes do torneio...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="px-4 py-8 text-center"> {/* Removed container mx-auto */}
        ...
        <h1 className="text-2xl font-semibold text-red-700 dark:text-red-400 mb-2">Erro ao Carregar Torneio</h1>
  ...
  if (!tournament) {
    return (
      <div className="px-4 py-8 text-center"> {/* Removed container mx-auto */}
        ...
        <h1 className="text-2xl font-semibold text-gray-700 dark:text-gray-200 mb-2">Torneio Não Encontrado</h1>
      ...
      <div className="bg-white dark:bg-slate-800 shadow-xl rounded-lg p-6 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          <div className="info-card p-4 bg-gray-50 dark:bg-slate-700 rounded-lg shadow">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">Início: {formatDate(tournament.date)}</p>
            <p className="text-sm text-gray-600 dark:text-gray-400">Fim: {tournament.endDate ? formatDate(tournament.endDate) : 'A definir'}</p>
          </div>
          <div className="info-card p-4 bg-gray-50 dark:bg-slate-700 rounded-lg shadow">
          ...
          <div className="info-card p-4 bg-gray-50 dark:bg-slate-700 rounded-lg shadow">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">{tournament.bracket_type?.replace('-', ' ') || 'Não definido'}</p>
          </div>
          <div className="info-card p-4 bg-gray-50 dark:bg-slate-700 rounded-lg shadow">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">Esperados: {tournament.num_players_expected || 'N/A'}</p>
          </div>
          <div className="info-card p-4 bg-gray-50 dark:bg-slate-700 rounded-lg shadow">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">Taxa: {tournament.entry_fee ? `R$ ${tournament.entry_fee}` : 'Grátis'}</p>
            <p className="text-sm text-gray-600 dark:text-gray-400">Prêmios: {tournament.prize_pool || 'Não definido'}</p>
          </div>
           <div className="info-card p-4 bg-gray-50 dark:bg-slate-700 rounded-lg shadow">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 whitespace-pre-wrap">{tournament.rules || 'Não definidas.'}</p>
          </div>
        </div>
        ...
        <div>
          <h2 className="text-2xl font-semibold text-gray-800 dark:text-gray-100 mb-4">Jogadores Inscritos</h2>
          {players.length > 0 ? (
            ...
            <p className="text-gray-500 dark:text-gray-400">Nenhum jogador inscrito neste torneio ainda.</p>
          )}
        </div>
