...
import { getTrashItems, restoreTrashItem, permanentlyDeleteDBItem } from '../../services/api'; // permanentlyDeleteDBItem já estava importado
...
const TrashPage = () => {
  ...
  const fetchTrash = useCallback(async (page = 1, type = '') => {
    ...
    try {
      ...
      showError(`Erro ao carregar lixeira: ${error.message || 'Erro desconhecido'}`); // Corrigido
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleRestore = async (itemId, itemType) => {
    if (window.confirm('Deseja restaurar este item?')) {
      try {
        ...
        showError(`Erro ao restaurar item: ${error.message || 'Erro desconhecido'}`); // Corrigido
      }
    }
  };
  ...
  const handlePermanentDelete = async (itemId, itemType) => {
    if (window.confirm('Deseja excluir permanentemente este item? Esta ação não pode ser desfeita.')) {
      try {
        ...
        showError(`Erro ao excluir item: ${error.message || 'Erro desconhecido'}`); // Corrigido
      }
    }
  };
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
              <thead className="bg-gray-50 dark:bg-slate-700">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Nome/Descrição</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Data Exclusão</th>
                ...
                {items.map((item) => (
                  <tr key={item.id + '-' + item.type}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                      <button onClick={() => handleRestore(item.id, item.type)} className="text-green-500 hover:text-green-400 dark:text-green-400 dark:hover:text-green-300 flex items-center" title="Restaurar">
                        <FaUndo className="mr-1" /> Restaurar
                      </button>
                      <button onClick={() => handlePermanentDelete(item.id, item.type)} className="text-red-500 hover:text-red-400 dark:text-red-400 dark:hover:text-red-300 flex items-center" title="Excluir permanentemente">
