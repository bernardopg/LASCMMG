...
export const MessageProvider = ({ children }) => {
  ...
  const removeMessage = useCallback((id) => {
    ...
    if (config.behavior === MESSAGE_BEHAVIORS.QUEUE && messageQueueRef.current.length > 0) {
      ...
      addMessageInternal(nextMessage.text, nextMessage.type, nextMessage.timeout, nextMessage.options);
    }
  }, [config.behavior]);

  // Função interna para adicionar mensagens (não exposta diretamente)
  const addMessageInternal = useCallback((text, type, timeout = config.defaultTimeout, options = {}) => {
