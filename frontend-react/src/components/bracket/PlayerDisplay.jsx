...
const PlayerDisplay = ({ player, score, isWinner, isChampion, isBye, seed }) => {
  ...
  let playerScoreClasses = "text-sm font-semibold ml-2 px-2 py-0.5 rounded-sm min-w-[24px] text-center"; // bracket-team-score
  ...
  } else if (score !== undefined && score !== null) { // Player has played but is not the winner
    playerNameClasses += " text-gray-400"; // Loser or TBD if score is present
    playerScoreClasses += " text-gray-300 bg-gray-600";
  } else { // Player has not played yet or score not set
  ...
  return (
    <div className={playerContainerClasses}>
      {seed && !isBye && <span className="text-xs text-gray-500 font-bold mr-2 w-5 text-center">({seed})</span>}
      ...
      {!isBye && (
        <span className={playerScoreClasses}>
          {playerScore}
        </span>
      )}
    </div>
