...
const BracketSection = ({ title, matches, roundPrefix, selectedMatchId, onMatchClick }) => {
  ...
  const roundOrder = [
    `${roundPrefix}Rodada 1`, `${roundPrefix}Rodada 2`, `${roundPrefix}Rodada 3`, `${roundPrefix}Rodada 4`,
    `${roundPrefix}Rodada 5`, `${roundPrefix}Rodada 6`, `${roundPrefix}Rodada 7`, `${roundPrefix}Rodada 8`,
    `${roundPrefix}Oitavas de Final`, `${roundPrefix}Quartas de Final`, `${roundPrefix}Semifinais`,
    `${roundPrefix}Final`, 'Grande Final', 'Grande Final Reset',
  ...
  return (
    <div className="mb-8 p-4 border border-gray-700 rounded-lg bg-gray-800 bg-opacity-50">
      <h3 className="text-xl font-semibold text-[var(--color-secondary)] mb-4 pb-2 border-b border-gray-600">{title}</h3>
      <div className="bracket-display flex flex-row overflow-x-auto pb-4 relative"> {/* Added relative for potential connector positioning */}
        {sortedRoundNames.map((roundName, roundIndex) => {
          const matchesForRound = matchesByRound[roundName].sort((a, b) => parseInt(a.id) - parseInt(b.id));
          return (
