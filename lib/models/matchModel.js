...
async function bulkCreateMatches(tournamentId, matchesDataArray) {
  ...
  await transactionAsync((db) => {
    ...
    const stmt = db.prepare(
      // Preparar statement sincronamente
      `
      INSERT INTO matches (tournament_id, match_number, round, player1_id, player2_id, scheduled_at, next_match, next_loser_match, bracket)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `
    ...
    for (const match of matchesDataArray) {
      ...
      const result = stmt.run(
        ...
        player2_id || null,
        scheduled_at || null,
        null, // next_match e next_loser_match são atualizados depois
        null,
        bracket
      );

      createdMatches.push({
        id: result.lastInsertRowid, // Correto para better-sqlite3
        tournament_id: tournamentId,
        match_number: match_number, // Usar o match_number original
        originalNextMatch: next_match, // Salvar para atualização posterior
        originalNextLoserMatch: next_loser_match, // Salvar para atualização posterior
        ...match,
      });
    }
