...
const { runAsync, getOneAsync } = require('../db');
...
async function getAdminByUsername(username) {
  ...
  const sql =
    'SELECT id, username, hashedPassword as hashed_password FROM users WHERE username = ?';
...
async function createAdmin(admin) {
  ...
  const sql = `
    INSERT INTO users (username, hashedPassword, created_at)
